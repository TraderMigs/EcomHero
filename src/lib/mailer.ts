...
export async function sendOrderConfirmation(
  props: Omit<OrderConfirmationProps, 'storeName' | 'accentColor' | 'storeUrl'> & { to: string }
) {
  try {
...
export async function sendLowStockAlert(
  props: Omit<LowStockAlertProps, 'storeName' | 'accentColor' | 'adminUrl'>
) {
  try {
    const resend = await getResend()
    if (!resend) return
...
export async function sendAbandonedCart(
  props: Omit<AbandonedCartProps, 'storeName' | 'accentColor' | 'storeUrl'> & { to: string }
) {
  try {
    const resend = await getResend()
    if (!resend) return

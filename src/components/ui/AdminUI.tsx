...
export function AdminPage({ children }: { children: React.ReactNode }) {
  return <div className="min-h-full flex flex-col gap-5 pb-8">{children}</div>
}
...
export function AdminPageHeader({ title, subtitle, action }: {
  ...
  return (
    <div className="flex items-start justify-between gap-4 pb-3 border-b" style={{ borderColor: palette.border }}>
      <div>
...
// ── Card ──────────────────────────────────────────────────────────────────────
export function AdminCard({ children, title, subtitle, className = '', padding = true }: {
  children: React.ReactNode; title?: string; subtitle?: string; className?: string; padding?: boolean
}) {
  return (
    <div className={`rounded-xl border overflow-hidden ${className}`} style={{ background: palette.surface, borderColor: palette.border }}>
      {(title || subtitle) && (
...
// ── Field ─────────────────────────────────────────────────────────────────────
export function AdminField({ label, hint, children, required }: {
...
export function AdminTextarea({ value, onChange, placeholder, rows = 4 }: {
  ...
  return (
    <textarea value={value} onChange={e => onChange(e.target.value)}
      ...
      className="w-full px-3 py-2.5 rounded-lg text-sm border outline-none resize-y transition-colors"
      style={{ background: palette.bg, borderColor: palette.border, color: palette.text }}
      onFocus={e => e.target.style.borderColor = palette.accent}
      onBlur={e => e.target.style.borderColor = palette.border}
    />
...
export function AdminSelect({ value, onChange, options, disabled }: {
  ...
  return (
    <select value={value} onChange={e => onChange(e.target.value)} disabled={disabled}
      ...
      style={{ background: palette.bg, borderColor: palette.border, color: palette.text }}>
      {options.map(o => <option key={o.value} value={o.value} style={{ background: palette.surface }}>{o.label}</option>)}
    </select>
  )
}
...
// ── Button ────────────────────────────────────────────────────────────────────
export function AdminButton({ children, onClick, variant = 'primary', size = 'md', disabled, loading, icon }: {
  children: React.ReactNode; onClick?: () => void
  variant?: 'primary' | 'secondary' | 'danger' | 'ghost'
  size?: 'sm' | 'md' | 'lg'; disabled?: boolean; loading?: boolean; icon?: React.ReactNode
}) {
  const styles = {
...
// ── Stat card ─────────────────────────────────────────────────────────────────
export function AdminStat({ label, value, icon, color = palette.accent }: {
  label: string; value: string | number; icon: React.ReactNode; color?: string
}) {
  return (
    <div className="rounded-xl border p-5 flex flex-col gap-3" style={{ background: palette.surface, borderColor: palette.border }}>
          ...
          <tr className="border-b" style={{ borderColor: palette.borderLight }}>
            {headers.map(h => (
              <th key={h} className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-widest"
                style={{ color: palette.textMuted, background: palette.bg }}>{h}</th>
            ))}
          </tr>
        </thead>
...
export function AdminTableRow({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) {
  return (
    <tr onClick={onClick} className="border-b transition-colors"
      style={{ borderColor: palette.borderLight, cursor: onClick ? 'pointer' : 'default' }}
      onMouseEnter={e => { if (onClick) (e.currentTarget as HTMLElement).style.background = palette.surfaceHover }}
...
export function AdminTableCell({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <td className={`px-5 py-3.5 ${className}`} style={{ color: palette.text }}>{children}</td>
}
...
export function AdminEmpty({ icon, title, subtitle, action }: {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center gap-3">
      <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: palette.border, color: palette.textMuted }}>{icon}</div>
      <div>
...
// ── Color picker ──────────────────────────────────────────────────────────────
export function AdminColorPicker({ value, onChange, label }: {
    ...
    <div className="flex items-center gap-3">
      <div className="relative">
        <input type="color" value={value} onChange={e => onChange(e.target.value)}
          className="absolute inset-0 w-10 h-10 rounded-lg border cursor-pointer opacity-0" />
        <div className="w-10 h-10 rounded-lg border-2 pointer-events-none" style={{ background: value, borderColor: palette.border }} />
      </div>
      <div>
...
export function AdminBackLink({ href, label }: { href: string; label: string }) {
  return (
    <a href={href} className="text-xs font-medium w-fit" style={{ color: palette.textMuted }}>
      ← {label}
    </a>
  )
}

...
function PreviewHero({ data, colors }: { data: Record<string, unknown>; colors: { primary: string; secondary: string; accent: string } }) {
  return (
    <div className="relative flex items-center justify-center overflow-hidden"
      style={{ minHeight: heights[(data.height as string)] || '55vh' }}>
      {data.image_url ? (
        <>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={data.image_url as string} alt="" className="absolute inset-0 w-full h-full object-cover" />
          <div className="absolute inset-0" style={{ background: `rgba(0,0,0,${((data.overlay_opacity as number) || 40) / 100})` }} />
        </>
      ) : (
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${colors.primary}ee, ${colors.primary}88)` }} />
      )}
      <div className="relative z-10 px-8 py-12 max-w-2xl w-full" style={{ textAlign: (data.text_align as 'left' | 'center' | 'right') || 'center' }}>
        {data.heading && <h1 className="text-3xl font-bold mb-3 leading-tight" style={{ color: data.image_url ? '#fff' : colors.secondary }}>{data.heading as string}</h1>}
        {data.subheading && <p className="text-base mb-6 opacity-80" style={{ color: data.image_url ? '#fff' : colors.secondary }}>{data.subheading as string}</p>}
        {data.cta_text && (
...
function PreviewSlideshow({ data, colors }: { data: Record<string, unknown>; colors: { primary: string; secondary: string; accent: string } }) {
  ...
  return (
    <div className="relative overflow-hidden" style={{ minHeight: heights[(data.height as string)] || '55vh' }}>
      {slide.image_url
        // eslint-disable-next-line @next/next/no-img-element
        ? <><img src={slide.image_url as string} alt="" className="absolute inset-0 w-full h-full object-cover" /><div className="absolute inset-0 bg-black/40" /></>
      ...
      <div className="relative z-10 flex items-center justify-center h-full px-8 py-16 text-center">
        <div>
          {slide.heading && <h2 className="text-3xl font-bold text-white mb-3">{slide.heading as string}</h2>}
          {slide.subheading && <p className="text-base text-white/80 mb-5">{slide.subheading as string}</p>}
          {slide.cta_text && <span className="inline-block px-5 py-2 text-xs font-bold text-white rounded" style={{ background: colors.accent }}>{slide.cta_text as string}</span>}
        </div>
      </div>
      {slides.length > 1 && (
        <>
...
function PreviewProductGrid({ data, products, colors }: { data: Record<string, unknown>; products: Product[]; colors: { primary: string; secondary: string; accent: string } }) {
  ...
  const display = (data.show_all as boolean)
    ? products.slice(0, (data.limit as number) || 8)
  ...
  return (
    <div className="px-5 py-10">
      {data.heading && <h2 className="text-2xl font-bold text-center mb-2" style={{ color: colors.primary }}>{data.heading as string}</h2>}
      {data.subheading && <p className="text-center text-sm opacity-60 mb-6">{data.subheading as string}</p>}
  ...
  return (
    <div className="px-5 py-8">
      <p className="mx-auto leading-relaxed text-sm opacity-75"
        style={{ color: colors.primary, textAlign: (data.align as 'left' | 'center' | 'right') || 'center', maxWidth: maxWidths[(data.max_width as string)] || '640px' }}>
  ...
  return (
    <div className="px-5 py-10">
      <div className={`flex flex-col md:flex-row items-center gap-6 max-w-4xl mx-auto ${!isLeft ? 'md:flex-row-reverse' : ''}`}>
        <div className="w-full md:w-2/5 aspect-square bg-gray-100 overflow-hidden shrink-0" style={{ borderRadius: radii[(data.image_shape as string)] || '12px' }}>
          {data.image_url
            // eslint-disable-next-line @next/next/no-img-element
            ? <img src={data.image_url as string} alt="" className="w-full h-full object-cover" />
            : <div className="w-full h-full flex items-center justify-center text-gray-200 text-3xl">🖼️</div>}
        </div>
        <div className="flex-1">
          {data.heading && <h2 className="text-2xl font-bold mb-3" style={{ color: colors.primary }}>{data.heading as string}</h2>}
          {data.body && <p className="text-sm opacity-70 leading-relaxed mb-4">{data.body as string}</p>}
          {data.cta_text && <span className="inline-block px-5 py-2 text-xs font-bold text-white rounded" style={{ background: colors.primary }}>{data.cta_text as string}</span>}
        </div>
      </div>
    </div>
  )
...
function PreviewTestimonials({ data, colors }: { data: Record<string, unknown>; colors: { primary: string } }) {
  ...
  return (
    <div className="px-5 py-10 bg-gray-50">
      {data.heading && <h2 className="text-2xl font-bold text-center mb-8" style={{ color: colors.primary }}>{data.heading as string}</h2>}
      <div className="grid grid-cols-3 gap-4 max-w-4xl mx-auto">
        {items.map((item, i) => (
          <div key={i} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex gap-0.5 mb-2">{Array.from({ length: 5 }).map((_, j) => <span key={j} className={`text-xs ${j < (item.rating as number) ? 'text-amber-400' : 'text-gray-200'}`}>★</span>)}</div>
            <p className="text-xs opacity-70 leading-relaxed mb-3">&ldquo;{item.quote as string}&rdquo;</p>
            <p className="text-xs font-semibold">{item.author as string}</p>
            <p className="text-xs opacity-40">{item.role as string}</p>
          </div>
        ))}
      </div>
    </div>
...
function PreviewFaq({ data, colors }: { data: Record<string, unknown>; colors: { primary: string } }) {
  ...
  return (
    <div className="px-5 py-10 max-w-2xl mx-auto">
      {data.heading && <h2 className="text-2xl font-bold text-center mb-6" style={{ color: colors.primary }}>{data.heading as string}</h2>}
      <div className="flex flex-col gap-2">
        {items.map((item, i) => (
          <div key={i} className="border border-gray-100 rounded-xl overflow-hidden">
            <button onClick={() => setOpen(open === i ? null : i)}
              className="w-full px-4 py-3 flex items-center justify-between text-left hover:bg-gray-50">
              <span className="text-xs font-semibold">{item.question as string}</span>
              <span className="text-gray-400 ml-3 shrink-0">{open === i ? '−' : '+'}</span>
            </button>
            {open === i && <div className="px-4 pb-3 text-xs opacity-60 leading-relaxed">{item.answer as string}</div>}
          </div>
        ))}
      </div>
    </div>
...
function PreviewCtaBanner({ data }: { data: Record<string, unknown> }) {
  return (
    <div className="px-5 py-14 text-center" style={{ background: (data.background_color as string) || '#000', color: (data.text_color as string) || '#fff' }}>
      {data.heading && <h2 className="text-2xl font-bold mb-2">{data.heading as string}</h2>}
      {data.subheading && <p className="text-sm opacity-80 mb-5">{data.subheading as string}</p>}
      {data.cta_text && <span className="inline-block px-6 py-2.5 text-xs font-bold border-2 border-current rounded">{data.cta_text as string}</span>}
    </div>
  )
}
  ...
  return (
    <div className="px-5 py-8 max-w-2xl mx-auto">
      <div className="aspect-video rounded-xl overflow-hidden shadow-lg">
        <iframe src={getEmbed(data.url as string)} className="w-full h-full" allowFullScreen title="video" />
      </div>
      {data.caption && <p className="text-center text-xs opacity-50 mt-2">{data.caption as string}</p>}
    </div>
  )
}
...
function PreviewDivider({ data, colors }: { data: Record<string, unknown>; colors: { primary: string } }) {
  ...
  const py = sp[(data.spacing as string)] || '32px'
  return (

...
import { AdminPage, AdminPageHeader, AdminTable, AdminTableRow, AdminTableCell, AdminEmpty } from '@/components/ui/AdminUI'
  ...
  return (
    <AdminPage>
      ...
      {customers && customers.length > 0 ? (
        <AdminTable headers={['Customer', 'Orders', 'Spent', 'Joined']}>
          {customers.map(c => (
            <AdminTableRow key={c.id}>
              ...
              <AdminTableCell><span className="text-xs" style={{ color: palette.textMuted }}>{new Date(c.created_at).toLocaleDateString()}</span></AdminTableCell>
            </AdminTableRow>
          ))}
        </AdminTable>
      ) : (
